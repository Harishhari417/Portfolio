...
export default function Navbar({ dark, setDark }) {
  return (
    <header className={dark ? "bg-gray-800 text-gray-100 shadow" : "bg-white text-gray-900 shadow-sm"}>
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-4">
          ...
          <button onClick={() => setDark(!dark)} className="ml-2 px-3 py-2 border rounded text-sm">
            {dark ? "Light" : "Dark"}
          </button>
        </nav>
      </div>
